...
# 研究発表の構成まで入れる場合
DECK=$(node "$FORGE/scripts/init.mjs" --topic paper_talk --title "論文紹介" --recipe paper-talk)

# ビジネス提案・経営報告の構成まで入れる場合
DECK=$(node "$FORGE/scripts/init.mjs" --topic customer_proposal --title "顧客向け提案" --recipe business-proposal)
```
...
| `--recipe` | いいえ | `research-progress` / `paper-talk` / `conference-talk` / `business-proposal` / `executive-review` |
...
単なる見た目ではなく「何を伝えるか」から選ぶ場合は `catalog/templates.json` を参照する。
研究向けには問題設定、RQ、貢献、手法、実験、結果、限界など、ビジネス向けには事業課題、
顧客像、提供価値、市場、競合、ビジネスモデル、KPI、リスク、アクションなどの意味的テンプレートを
`templates/` に同梱している。これらはAgentがレシピ生成時に利用し、編集WebUIには一覧表示しない。
テンプレートの `copy` に記載された素材は、レシピ初期化時にデッキの `assets/` へコピーする。
...
| `chart-insight` | グラフ左+結論と解釈右(画像+blockquote+箇条書き)※research |
| `qualitative-grid` | 手法×入力例の定性結果画像グリッド(表ベース)※research |
...
- グラフ+最重要の解釈 → `chart-insight`、定性画像の手法比較 → `qualitative-grid`
...
### 2.1 図・グラフ・テンプレート素材を用意する

再現可能な図表は `scripts/render-visual.mjs` で YAML/JSON からSVGへ生成する。

```bash
node "$FORGE/scripts/render-visual.mjs" visual.yaml --theme research --output assets/result.svg
```

対応グラフは bar / line / scatter / area / heatmap、構成図は pipeline / architecture /
sequence。仕様例は `assets/examples/`、スキーマは `assets/visual.schema.json` を参照する。
`alt` は必須。`motion: fade|wipe|draw|highlight` は rich HTML 用で、静的出力では最終状態になる。
`highlight` は同名の静的 `highlight` 指定があれば対象マークだけを1回強調し、対象を特定できない図では図全体を1回強調する。
生成SVGと仕様ファイルのbasenameを揃えると、品質チェックが軸・単位・凡例を検査できる。
別名の場合は `<!-- sf_visual_spec: data/result.yaml -->` で対応を明示する。

写真・アイコンが必要な場合は `assets/assets.json` から選ぶ。ここに載る素材はローカル同梱され、
作者・配布元・ライセンス・取得日・SHA-256・alt・用途タグを持つ。WebUIの「素材」から選ぶと
デッキへコピーされ、`sources/assets.json` に provenance が残る。リモートURLをMarkdownへ
直接貼らない。

同梱素材で意味が合わない概念図・キービジュアルは、外部画像生成APIをアプリへ組み込まず、
**Codexの組み込みImageGen**を使う。次を明記した画像ブリーフを先に作る:

- スライドでの役割、描く対象、構図、16:9または必要な比率
- テキストを置く安全余白、スキンの色調、低彩度/エディトリアル・ミニマル
- 画像内の文字・ロゴ・透かし・誤解を招く科学的描写を禁止

生成結果は `assets/` に保存し、同名の `*.prompt.md` にブリーフ・生成日時・altを残す。
ImageGenが使えない環境では同梱素材かプレースホルダーにフォールバックする。
...
```bash
# 1. オフライン単一HTMLをビルド(画像パス切れを防ぐ)
node "$FORGE/scripts/export.mjs" slides.md --format html --output build/slides.html
# 2. 機械チェック(はみ出し検出)
node "$FORGE/scripts/check-overflow.mjs" build/slides.html
# 2b. 内容・画像・アクセシビリティ品質(警告も失敗にする場合は --strict)
node "$FORGE/scripts/check-quality.mjs" slides.md build/slides.html
# 発表者ノートと素材出典も必須にする場合
node "$FORGE/scripts/check-quality.mjs" slides.md --require-notes --require-provenance
# 3. PNG化して目視確認
...
`check-quality.mjs` は画像欠損/外部URL/alt、タイトル、情報密度、文字サイズ、コントラスト、
同一レイアウトの過剰な連続に加え、明示された図表仕様の軸・単位・凡例、素材のprovenance、
発表者ノートを検査する。誤検知を抑える適用条件・source recordの形式・`sf_chart` 記法は
`references/quality.md` を参照する。警告は内容を確認し、意図した例外だけを残す。

### 4.1 スライド内モーションと書き出し

frontmatter の `sf_motion` は、一枚のスライド内にある要素の表示だけを制御する:

```yaml
sf_motion: standard  # off / standard / rich
```

- `standard`: 通常表示(既定)。独自のrich runtimeは注入しない
- `rich`: 要素の入場と `![sf:fade 説明](...)` / `sf:wipe` / `sf:draw` / `sf:highlight` を有効化
- `off`: 動きを使わない意図を明示する互換モード

スライド間のView Transitionは常に無効化する。`draw` はSVG内の `data-sf-draw` 対象を描画し、
対象がなければ図全体のwipeへフォールバックする。`highlight` は生成図の
`data-sf-highlight` 対象を強調し、対象がなければ図全体を強調する。

`prefers-reduced-motion` と印刷では動きを止め、すべて最終状態で表示する。書き出しは共通CLIを使う:

```bash
node "$FORGE/scripts/export.mjs" slides.md --format html
node "$FORGE/scripts/export.mjs" slides.md --format pdf
```

HTMLは画像・SVG・KaTeXフォント・rich runtimeをインライン化した発表用のオフライン単一ファイル、
PDFは配布・確認用の静的出力になる。発表者ノートはMarkdown内に保持し、単体ファイルには書き出さない。
...
- 発表用オフラインHTMLとPDFは WebUI のボタン、または `scripts/export.mjs` を使う。テンプレート選択と詳細な品質判断はAgent側で行う
