...
| `cards` | カード配置(アイコン一覧・機能一覧、4〜8項目。columns より多項目・複数行折り返し) |
| `scorecard` | スコアカード(評価基準ごとの数値評価。「**基準** 説明 *スコア*」) |
| `transition` | Before・After / As-Is・To-Be の文章版(2列+中央に方向矢印) |
| `changelog` | 変更履歴・更新履歴(バージョン+日付+変更点、新しい版から) |
| `contact` | 連絡先(終了スライドの発展形、ラベル+値を中央寄せ) |
| `experiment` | 実験結果(表+条件注記は blockquote)※research |
| `math` | 数式中心(KaTeX ディスプレイ数式を拡大)※research |
| `references` | 参考文献(小さめフォント)※research |
| `definition` | 定義・定理(用語+枠付き定義文+任意の例)※research |
| `kpi` | 数値ハイライト(大きく軽い数字、最大4つ)※business |
| `plans` | 料金プラン・パッケージ比較(2〜3列)※business |
| `persona` | ペルソナ(顔写真+ゴール・課題等のラベル一覧)※business |
| `tam-sam-som` | 市場規模(TAM・SAM・SOMの同心円3層)※business |
| `case-study` | 導入事例(課題→効果+顧客の声)※business |
| `journey` | カスタマージャーニー(ステージ×タッチポイント/感情、3〜5段)※business |
...
- グラフ(棒・折れ線・散布図等) → 作図した画像(SVG/PNG)を `image-full`(1枚)/
  ...
  CSSでグラフは描かない。分岐フローや複雑な構成図も同様に画像を貼る
...
- 4象限の整理(SWOT等) → `matrix`、3×3(リスク評価・優先度) → `matrix-3`、2集合の関係 → `venn`、階層 → `pyramid`
...
- 用語・定理の定義 → `definition`(research)
- 想定ユーザー像の整理 → `persona`(business)
- 市場規模(TAM/SAM/SOM) → `tam-sam-som`(business)
...
- 色・フォント・作図時のグラフパレットの正は `~/projects/slide-forge/docs/DESIGN.md` を参照
