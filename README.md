...
- 左: 実スライドの縮小サムネイル(クリック選択・ドラッグ並び替え・複製/削除)。
  右端の境界線をドラッグして幅を調整できる(160〜480px、次回起動時も記憶)
- スライド追加は分割ボタン「＋追加 ▾」: 本体クリックでデフォルトレイアウト
  (`content`)のスライドを追加、▾ クリックでプレビュー付きピッカーが開き
  レイアウトを選べる(選ぶとそのレイアウトの書き方例がプレースホルダ内容として
  入った新規スライドが挿入される)
