...
- **リファレンス** コミットIDに付けた名前。ブランチもタグもこれ
- **オブジェクト** .git/objects に保存される不変データの総称

今日はこの5語が分かっていれば付いてこられます
...
![Gitオブジェクトを読み出す処理シーケンス](assets/charts/lecture-sequence.svg)
...
![slide-forge編集WebUIの画面](assets/screenshots/webui-editor.png)
...
1. ![Gitオブジェクトを読み出す処理シーケンス](assets/charts/lecture-sequence.svg) **リポジトリ作成** `git init demo` で空のリポジトリを作る
2. ![slide-forge編集WebUIの画面](assets/screenshots/webui-editor.png) **コミット作成** ファイルを追加して `git commit` する
3. ![Gitオブジェクトを読み出す処理シーケンス](assets/charts/lecture-sequence.svg) **中身を覗く** `.git/objects` を `cat-file` で辿る
